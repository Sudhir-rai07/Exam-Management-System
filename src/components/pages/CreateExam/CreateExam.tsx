...
const CreateExam = () => {
  ...
  const getNavItemStle = (url: string) =>{
    ...
    return location.pathname.includes(url) ? "text-blue-500 font-semibold" : "";
  }

  return (
    <section className="w-full h-full">
      {/* CreateExam Navigation */}
      <div className={`md:flex hidden  items-center flex-wrap justify-start md:justify-center w-full gap-6 py-4 mx-auto overflow-x-scroll`}>
        {examCreationPhases.map((phase: {name: string, url: string}, idx: number) => (
          <Link to={phase.url} key={idx} className={`flex items-center ${getNavItemStle(phase.url)}`}>
      ...
      <DropdownMenu>
            ...
            <DropdownMenuContent>
              ...
              {examCreationPhases.map((phase: {name: string, url: string}, idx: number) => (
          <DropdownMenuItem  key={idx} className={`flex items-center ${getNavItemStle(phase.url)}`}>
